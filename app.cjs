...
const {beginLogin,finishLogin}=require('./steam.cjs');
...
function createApp(options={}){
 ...
 function cleanExpired(){db.prepare('DELETE FROM sessions WHERE expires_at<?').run(now());db.prepare('DELETE FROM logins WHERE expires_at<?').run(now());db.prepare('DELETE FROM devices WHERE expires_at<?').run(now());db.prepare('DELETE FROM nonces WHERE expires_at<?').run(now());db.prepare('DELETE FROM admin_sessions WHERE expires_at<?').run(now());db.prepare('DELETE FROM admin_logins WHERE expires_at<?').run(now());}
 const admin=require('./admin.cjs').createAdmin({db,origin,secret,ownerSteamId:options.ownerSteamId||'',storageBytes:budget,send,json,limit,steamFetch:options.steamFetch,onError:options.onError});
 ...
 async function handler(req,res){
  ...
  try{
   ...
   if(await avatars.handle(req,res,u))return;
    if(method==='GET'&&p==='/api/info')return send(res,200,{name:options.name||'Zoigram',version:require('../package.json').version,environment:local?'local':'public',authentication:'steam',features:{avatars:true},languages:I18n.languages,limits:{imageBytes:MAX_IMAGE,caption:2200,comment:1000,message:2000,postsPerDay:20},origin});
   ...
   if(method==='GET'&&p==='/connect'){
    const code=(u.searchParams.get('code')||'').toUpperCase();if(!/^[0-9A-F]{10}$/.test(code)||!db.prepare('SELECT 1 FROM devices WHERE user_code=? AND expires_at>? AND consumed=0 AND profile_id IS NULL').get(code,now()))fail(400,'Код входа истёк. Начните заново в игре.');
    return send(res,200,page(I18n.t(language,'Войти в Zoigram'),'<p>'+t('Сверьте код с кодом в игре:')+'</p><p><code>'+code+'</code></p><p>'+t('Продолжайте, только если вы сами начали этот вход. Один Steam-аккаунт — один профиль Zoigram.')+'</p><a href="/auth/steam?code='+code+'&amp;lang='+language+'">'+t('Продолжить через Steam')+'</a><p><small>'+t('Пароль вводится только на сайте Steam. Zoigram его не получает. Публикации доступны другим зарегистрированным игрокам.')+'</small></p>',language),'text/html');
   }
   if(method==='GET'&&p==='/auth/steam'){
    limit('steam:'+ip,10,600000);const start=beginLogin(db,origin,u.searchParams.get('code'));res.setHeader('Set-Cookie',['zg_login='+start.cookie+'; HttpOnly; SameSite=Lax; Path=/auth/steam/callback; Max-Age=600'+(local?'':'; Secure'),'zg_lang='+language+'; HttpOnly; SameSite=Lax; Path=/; Max-Age=3600'+(local?'':'; Secure')]);res.writeHead(303,{Location:start.url});return res.end();
   }
   if(method==='GET'&&p==='/auth/steam/callback'){
    const cookie=(req.headers.cookie||'').match(/(?:^|;\s*)zg_login=([A-Za-z0-9_-]{43})(?:;|$)/)?.[1];await finishLogin(db,origin,u.searchParams,cookie,options.steamFetch||fetch);res.setHeader('Set-Cookie','zg_login=; HttpOnly; SameSite=Lax; Path=/auth/steam/callback; Max-Age=0'+(local?'':'; Secure'));return send(res,200,page(I18n.t(language,'Вход подтверждён'),'<p>'+t('Вернитесь в Zoigram в игре. Этот профиль доступен со всеми вашими персонажами и сохранениями.')+'</p>',language),'text/html');
   }
   if(method==='GET'&&p==='/')return send(res,200,page('Zoigram','<p>'+t('Фотографии из inZOI, которыми делятся реальные игроки.')+'</p><p>'+t('Для входа откройте Zoigram в телефоне персонажа, нажмите «Войти через Steam».')+'</p>'+(local?'<p><small>'+t('Сейчас это локальный тестовый сервер на вашем компьютере.')+'</small></p>':''),language),'text/html');
