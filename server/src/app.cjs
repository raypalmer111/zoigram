...
function createApp(options={}){
 ...
 const admin=require('./admin.cjs').createAdmin({db,origin,secret,zoimeetMonitor:options.zoimeetMonitor,ownerSteamId:options.ownerSteamId||'',ownerProfileId:options.ownerProfileId||'',accounts,storageBytes:budget,send,json,limit,onError:options.onError});
 ...
 async function handler(req,res){
  ...
  try{
   ...
   if(p.startsWith('/api/')){res.setHeader('Access-Control-Allow-Origin','*');res.setHeader('Access-Control-Allow-Headers','Authorization, Content-Type, Accept-Language, X-Zoigram-Features');res.setHeader('Access-Control-Allow-Methods','GET, POST, PUT, PATCH, DELETE, OPTIONS');if(method==='OPTIONS'){res.writeHead(204);return res.end()}}
   ...
   if(await accounts.handle(req,res,u,ip,language))return;
    if(method==='GET'&&p==='/api/info')return send(res,200,{name:options.name||'Zoigram',version:require('../package.json').version,environment:local?'local':'public',authentication:'password',features:{avatars:true,accounts:true,profileSearch:true,captionEditing:true,savedPosts:true,commentNotifications:true},languages:I18n.languages,limits:{imageBytes:MAX_IMAGE,caption:2200,comment:1000,message:2000,postsPerDay:20},origin});
   ...
   if(method==='GET'&&p==='/api/activity')return send(res,200,{unreadNotifications:unreadNotifications(uid,commentAlerts),unreadMessages:unreadMessages(uid)});
   if(method==='GET'&&p==='/api/notifications'){
    ...
    const rows=db.prepare(`SELECT n.* FROM notifications n JOIN profiles a ON a.id=n.actor_id WHERE n.profile_id=? AND n.id<? AND (? OR n.kind!='comment') AND a.banned=0 AND NOT EXISTS(SELECT 1 FROM blocks b WHERE (b.blocker_id=? AND b.blocked_id=n.actor_id) OR (b.blocker_id=n.actor_id AND b.blocked_id=?)) ORDER BY n.id DESC LIMIT 21`).all(uid,before?Number(before):Number.MAX_SAFE_INTEGER,commentAlerts?1:0,uid,uid);
    return send(res,200,{notifications:rows.slice(0,20).map(n=>notificationDto(n,uid)),nextCursor:rows.length>20?String(rows[19].id):null,unread:unreadNotifications(uid,commentAlerts)});
   }
   if(method==='PUT'&&p==='/api/notifications/read'){
    const b=await json(req),through=Number(b.through);if(!Number.isSafeInteger(through)||through<1)fail(400,'Неверный номер уведомления.');db.prepare("UPDATE notifications SET read_at=COALESCE(read_at,?) WHERE profile_id=? AND id<=? AND (? OR kind!='comment')").run(now(),uid,through,commentAlerts?1:0);return send(res,200,{ok:true,unread:unreadNotifications(uid,commentAlerts)});
   }
   ...
   if(method==='GET'&&p==='/api/saved'){
    ...
    return send(res,200,{posts:rows.slice(0,10).map(p=>postDto(p,s)),nextCursor:rows.length>10?String(rows[9].bookmark_id):null});
   }
