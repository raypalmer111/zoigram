...
function createOperations({db,budget,gate,dataDirectory,backupDirectory,clock=Date.now}){
 ...
 function record({source='server',profileId=null,clientVersion,code,status=0,imageBytes=null,route='upload'}){
  ...
  const id=Number(db.prepare('INSERT INTO operational_errors(created_at,profile_id,source,code,status,client_version,image_bytes,route) VALUES(?,?,?,?,?,?,?,?)').run(clock(),profileId,source,label,Number.isInteger(status)&&status>=0&&status<=599?status:0,version(clientVersion),n,ROUTES.has(route)?route:'api').lastInsertRowid);
 ...
 function capture(req,res,u,status,error){
  ...
  const route=u.pathname==='/api/posts'&&req.method==='POST'?'upload_legacy':/^\/api\/uploads(?:\/|$)/.test(u.pathname)?'upload_resumable':u.pathname.startsWith('/api/auth/')?'auth':u.pathname.startsWith('/api/media/')?'media':'api';
  try{return record({profileId:res.zoigramSession?.profile_id||null,clientVersion:req.headers['x-zoigram-version'],code:route.startsWith('upload_')?error?.code:undefined,status,imageBytes:req.zoigramImageBytes,route});}catch{return null;}
 }
 ...
 function snapshot(){
  ...
  const errorSummary=db.prepare(`SELECT COUNT(*) total,COALESCE(SUM(source='server' AND status>=500),0) server5xx,COALESCE(SUM(source='server' AND code='busy' AND status=503),0) busyResponses,COALESCE(SUM(source='server' AND ((code IN ('upload_aborted','upload_cancelled') AND status=499) OR (code='upload_timeout' AND status=408))),0) uploadConnections,COALESCE(SUM(source='server' AND status>=400 AND status<500 AND code NOT IN ('upload_aborted','upload_cancelled','upload_timeout')),0) rejected4xx,COALESCE(SUM(source='client'),0) clientDiagnostics FROM operational_errors WHERE created_at>?`).get(clock()-86400000);
