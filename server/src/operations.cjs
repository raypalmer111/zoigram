...
function createOperations({db,budget,gate,dataDirectory,backupDirectory,clock=Date.now}){
 ...
 function capture(req,res,u,status,error){
  ...
  try{return record({profileId:res.zoigramSession?.profile_id||null,clientVersion:req.headers['x-zoigram-version'],code:route.startsWith('upload_')||route==='media'?error?.code:undefined,status,imageBytes:req.zoigramImageBytes,route});}catch{return null;}
 }
