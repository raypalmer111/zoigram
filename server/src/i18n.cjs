'use strict';
const catalog=require('./locales.json'),languages=Object.keys(catalog),accountLanguage='en';
...
function languageFor(req,url){
 // Account pages always use English, including old links whose lang is rewritten by the game.
 if(url.pathname==='/connect'||url.pathname==='/account'||url.pathname.startsWith('/account/'))return accountLanguage;
...
module.exports={languages,accountLanguage,normalize,negotiate,languageFor,t,catalog};
