...
const app=createApp({database:path.join(data,'zoigram.sqlite'),dataDirectory:data,backupDirectory:path.resolve(process.env.BACKUP_DIR||path.join(__dirname,'../backups')),origin,secret:fs.readFileSync(secretPath),name:process.env.COMMUNITY_NAME||'Zoigram',ownerSteamId:process.env.OWNER_STEAM_ID||'',ownerProfileId:process.env.OWNER_PROFILE_ID||'',trustProxy:process.env.TRUST_PROXY==='loopback',storageBytes:Number(process.env.STORAGE_LIMIT_MB||10240)*1024*1024,onError:e=>console.error('[Zoigram]',e.stack)});
