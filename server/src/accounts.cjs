...
function createAccounts({db,origin,secret,fail,limit,json,send,authorize}){
 ...
 async function form(req){if(!/^application\/x-www-form-urlencoded(?:;|$)/i.test(req.headers['content-type']||''))fail(415,'Неверный формат формы.');let size=0,chunks=[];for await(const c of req){size+=c.length;if(size>8192)fail(413,'Слишком большой запрос.');chunks.push(c)}const params=new URLSearchParams(Buffer.concat(chunks).toString('utf8')),result={};for(const [key,value]of params){if(Object.hasOwn(result,key))fail(400,'Неверный формат формы.');result[key]=value}return result}
