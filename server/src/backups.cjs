...
function inspectDatabase(file){
 ...
 try{
  ...
  if(db.prepare('PRAGMA foreign_key_check').all().length)throw Error('SQLite relationships are damaged');
   ...
   const counts={};for(const table of ['profiles','posts','comments','likes','follows','notifications','direct_messages','moderation','avatars','post_photos','pinned_posts','public_id_aliases','creator_grants'])if(present.has(table))counts[table]=db.prepare('SELECT COUNT(*) n FROM '+table).get().n;
