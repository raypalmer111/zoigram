...
const MAX_INPUT=25*1024*1024,TTL=24*3600000,EMPTY_GRACE=2*60000;
function createUploads({db,budget,fail,json,send,key,text,limit,transport,authorize,postDto,visiblePost,onPostCreated=()=>{},clock=Date.now}){
 const stagedLimit=Math.min(512*1024*1024,Math.floor(budget/4));
 const active=new Map(),activeKey=r=>r.profile_id+':'+r.request_id+':'+r.generation,isActive=r=>(active.get(activeKey(r))||0)>0;
 function hold(r){const k=activeKey(r);active.set(k,(active.get(k)||0)+1);return ()=>{const n=active.get(k)-1;if(n)active.set(k,n);else active.delete(k);};}
 const row=(uid,id)=>db.prepare('SELECT * FROM upload_sessions WHERE profile_id=? AND request_id=?').get(uid,id);
 function clean(){const remove=db.prepare('DELETE FROM upload_sessions WHERE profile_id=? AND request_id=? AND generation IS ?');for(const r of db.prepare('SELECT * FROM upload_sessions WHERE post_id IS NULL AND expires_at<=?').all(clock()))if(!isActive(r))remove.run(r.profile_id,r.request_id,r.generation);}
 function reclaimEmpty(uid){
  const candidates=db.prepare('SELECT s.* FROM upload_sessions s WHERE s.post_id IS NULL AND s.created_at<=? AND (? IS NULL OR s.profile_id=?) AND NOT EXISTS(SELECT 1 FROM upload_parts p WHERE p.profile_id=s.profile_id AND p.request_id=s.request_id) ORDER BY s.created_at').all(clock()-EMPTY_GRACE,uid,uid);
  const remove=db.prepare('DELETE FROM upload_sessions WHERE profile_id=? AND request_id=? AND generation IS ?');for(const r of candidates)if(!isActive(r))remove.run(r.profile_id,r.request_id,r.generation);
 }
 function waitForSlot(uid){const at=clock(),rows=db.prepare('SELECT s.*,EXISTS(SELECT 1 FROM upload_parts p WHERE p.profile_id=s.profile_id AND p.request_id=s.request_id) has_parts FROM upload_sessions s WHERE s.profile_id=? AND s.post_id IS NULL').all(uid);return Math.max(1,Math.ceil(Math.min(...rows.map(r=>isActive(r)?60000:Math.max(1,(r.has_parts?r.expires_at:Math.min(r.expires_at,r.created_at+EMPTY_GRACE))-at)))/1000));}
 function get(uid,id){const r=row(uid,id);if(!r||(!r.post_id&&r.expires_at<=clock()))fail(410,'Загрузка истекла. Повторите отправку.',{code:'upload_expired'});return r;}
 function status(r,s){return {requestId:r.request_id,received:db.prepare('SELECT position FROM upload_parts WHERE profile_id=? AND request_id=? ORDER BY position').all(r.profile_id,r.request_id).map(p=>p.position),expiresAt:r.expires_at,emptyOnlyCancellation:true,...(r.post_id?{post:postDto(visiblePost(r.post_id,s.profile_id),s)}:{})};}
 ...
 async function handle(req,res,u,s){
  ...
  if(m==='POST'&&!id){
   ...
   if(r){if(r.caption!==caption||r.photo_count!==count)fail(409,'Этот запрос уже использован для другого поста.');}
    ...
    reclaimEmpty(uid);
    if(db.prepare('SELECT COUNT(*) n FROM upload_sessions WHERE profile_id=? AND post_id IS NULL').get(uid).n>=3)fail(429,'Завершите предыдущую загрузку или повторите позже.',{code:'upload_slots_full',retryAfter:waitForSlot(uid),stage:'upload_start'});
    if(db.prepare('SELECT COUNT(*) n FROM upload_sessions WHERE post_id IS NULL').get().n>=300)reclaimEmpty(null);
    if(db.prepare('SELECT COUNT(*) n FROM upload_sessions WHERE post_id IS NULL').get().n>=300)fail(503,'Сервис занят. Попробуйте позже.',{code:'upload_service_full',retryAfter:60,stage:'upload_start'});
  ...
  const r=get(uid,id);
  function currentGeneration(){const current=get(uid,id);if(current.generation!==r.generation)fail(409,'Загрузка изменена. Повторите отправку.',{code:'upload_changed'});return current;}
  if(m==='GET'&&!part){send(res,200,status(r,s));return true;}
  if(m==='DELETE'&&!part){
   const b=Number(req.headers['content-length'])>0||req.headers['transfer-encoding']?await json(req,4096):{};authorize(req);const current=currentGeneration();
   if(current.post_id)fail(409,'Публикация уже отправлена.');
   if(b.emptyOnly===true){if(isActive(current))fail(409,'Фотография ещё отправляется. Повторите позже.',{code:'upload_in_progress',stage:'upload_cancel'});if(db.prepare('SELECT 1 FROM upload_parts WHERE profile_id=? AND request_id=? LIMIT 1').get(uid,id))fail(409,'Загруженные фотографии сохранены. Продолжите отправку.',{code:'upload_has_photos',stage:'upload_cancel'});}
   db.prepare('DELETE FROM upload_sessions WHERE profile_id=? AND request_id=?').run(uid,id);send(res,200,{ok:true});return true;
  }
  if(m==='PUT'&&/^[0-4]$/.test(part||'')){
   ...
   limit('upload-part:'+uid,120,3600000);const lease=transport.admit(req,res),release=hold(r);
   try{
   ...
   }finally{release();lease.release();}
  }
  if(m==='POST'&&part==='complete'){
   limit('upload-complete:'+uid,60,3600000);const release=hold(r);try{await json(req,4096);authorize(req);
   const postId=transaction(db,()=>{
    ...
    const daily=db.prepare('SELECT COUNT(*) n,MIN(created_at) oldest FROM posts WHERE profile_id=? AND created_at>?').get(uid,clock()-86400000);
    if(daily.n>=20)fail(429,'Достигнут лимит публикаций на сегодня.',{code:'daily_posts_limit',retryAfter:Math.max(1,Math.ceil((daily.oldest+86400000-clock())/1000)),stage:'upload_complete'});
    ...
    db.prepare('UPDATE upload_sessions SET post_id=? WHERE profile_id=? AND request_id=?').run(post,uid,id);db.prepare('DELETE FROM upload_parts WHERE profile_id=? AND request_id=?').run(uid,id);onPostCreated(post,uid,current.caption);return post;
   });send(res,200,{post:postDto(visiblePost(postId,uid),s)});return true;}finally{release();}
  }
  fail(405,'Метод не поддерживается.');
 }
 return {handle,clean,stagedLimit};
}
module.exports={createUploads,MAX_INPUT,TTL,EMPTY_GRACE};
