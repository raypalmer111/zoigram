...
test('schema 6 migration retains IDs, read states and monotonic notification cursors',t=>{
 ...
 try{assert.equal(db.prepare('PRAGMA user_version').get().user_version,8);assert.deepEqual(db.prepare('SELECT id,profile_id,actor_id,kind,post_id,created_at,read_at FROM notifications').all(),before);assert.equal(identity(db,'test','a').id,a.id);const next=db.prepare("INSERT INTO notifications(profile_id,actor_id,kind,created_at) VALUES(?,?,?,?)").run(b.id,a.id,'follow',5).lastInsertRowid;assert(next>100,'Do not reuse deleted IDs already acknowledged by clients');assert.deepEqual(db.prepare('PRAGMA foreign_key_check').all(),[])}finally{db.close()}
