...
test('schema 7 photos and IDs survive migration; backup restores all album photos',async t=>{
 ...
 const f=await fixture(t,{database:dbFile});assert.equal(f.app.db.prepare('PRAGMA user_version').get().user_version,11);assert.equal(f.app.db.prepare('SELECT caption FROM posts WHERE id=1').get().caption,'Keep');
