...
function fixture(t){
 ...
 t.after(()=>{db.close();assert(path.resolve(root).startsWith(path.resolve(os.tmpdir())+path.sep+'zoigram-backup-'));fs.rmSync(root,{recursive:true,force:true,maxRetries:5,retryDelay:100})});
...
test('a pre-0.14 snapshot verifies and migrates without inventing tables in its signed manifest',async t=>{
 ...
 const target=path.join(f.root,'legacy-restored');await restoreBackup(made.directory,target);const migrated=openStore(path.join(target,'zoigram.sqlite'));try{assert.equal(migrated.prepare('PRAGMA user_version').get().user_version,10);assert.equal(migrated.prepare('SELECT COUNT(*) n FROM notifications').get().n,0);assert.equal(migrated.prepare('SELECT COUNT(*) n FROM direct_messages').get().n,0);assert.equal(migrated.prepare('SELECT COUNT(*) n FROM posts').get().n,1)}finally{migrated.close()}
});
...
test('missing source database is refused without creating an empty replacement',async t=>{
 const root=fs.mkdtempSync(path.join(os.tmpdir(),'zoigram-backup-'));t.after(()=>{assert(path.resolve(root).startsWith(path.resolve(os.tmpdir())+path.sep+'zoigram-backup-'));fs.rmSync(root,{recursive:true,force:true,maxRetries:5,retryDelay:100})});
